...
var payment = {
	paymentdata: [{
		...
		{
			"name": "微信支付",
			"src": "images/sahngpin/weixin.png",
		},
		{
			"name": "",
			"src": "images/sahngpin/zhifubao.png",
		},
	]
}
var invoice = {
...
var vm = new Vue({
	...
	data: {
		...
		userBuyData: [], //用户购买数据
		options:options.optiondata,//收货地址所在地
		...
		saveData: function() {
			...
			this.shopTableDatas.forEach(function(item, index) {
				if(item.checked == true) {
					...
					window.location.href = 'tijiaodindan.html'
				}
			})
